...
def source_line():
    return config['source_proxy'].format(proxy_profile=config['proxy_file'])
...
def check_and_prevent_proxy_disable():
    ...
    for service in services:
        ...
        if (not http["enabled"] or http["server"] != config['host_listen'] or http["port"] != config['port_listen'] or
            not https["enabled"] or https["server"] != config['host_listen'] or https["port"] != config['port_listen']):
...
def watch_proxy_env_file_and_system_proxy():
    ...
    if not os.path.exists(proxy_env_path):
        print(f"{config['proxy_file']} does not exist. Creating...")
        create_proxy_profile()
        ...
        try:
            if os.path.exists(proxy_env_path):
                ...
                if last_mtime is None or current_mtime != last_mtime:
                    print(f"{config['proxy_file']} modification detected or monitoring started.")
                ...
                print(f"{config['proxy_file']} not found. Restoring...")
                create_proxy_profile()
                check_and_restore_source_lines()
                last_mtime = os.path.getmtime(proxy_env_path) if os.path.exists(proxy_env_path) else None
